...
from .features.cc_features import (
...
from .features.morph_based_features import erosion, closing, opening
from .features.noise_removal_based_features import gaussian, median
from .features.spatial_characteristic_features import (
    foreground_percent,
    gradients
)
...
class ParsedImage:
    ...
    def __init__(self, image: np.array, name: str = '', min_height=5, max_height=20, min_width=2,
                 max_width=100) -> None:
        # import time
        self.name = name
        self.image = image
        self.width = image.shape[0]
        self.thresh = self.get_threshold(True)
        # start_time = time.perf_counter()
        self.inversed = self.get_inversed()
        self.df_cc, self.labels_cc = self.get_bcc()
        self.df_wcc, self.labels_wcc = self.get_wcc()
        self.df_char, self.labels_filtered = self.filter_cc(min_height, max_height, min_width, max_width)
